...
| Stripe | Payment processing and refund/settlement system | Authenticated webhook events for refund creation and updates; refund statuses include pending, requires_action, succeeded, failed, and canceled | First provider connector; normalize authenticated refund events into evidence while disclosing the relay trust boundary | IMPLEMENTED | Live Stripe test-mode webhook exercise or API lifecycle change |
